 ...
 *
 * Felix Erlacher:
 * This file is copied from the PF_RING git repo
 * struct names changed to pfring_* to avoid clashes with netinet structs
 *
 */
//TODO: ev. update this file for future releases of PF_RING
...
static int __pfring_parse_tunneled_pkt(u_char *data, struct pfring_pkthdr *hdr, u_int16_t ip_version, u_int16_t tunnel_offset) {
  ...
  if(ip_version == 4 /* IPv4 */ ) {
    struct pfring_iphdr *tunneled_ip;

    if(data_len < (tunnel_offset+sizeof(struct pfring_iphdr)))
      return 0;

    tunneled_ip = (struct pfring_iphdr *) (&data[tunnel_offset]);
  ...
  if(hdr->extended_hdr.parsed_pkt.tunnel.tunneled_proto == IPPROTO_TCP) {
    struct pfring_tcphdr *tcp;

    if(data_len < tunnel_offset + sizeof(struct pfring_tcphdr))
      return 1;

    tcp = (struct pfring_tcphdr *)(&data[tunnel_offset]);
    ...
    struct pfring_udphdr *udp;

    if(data_len < tunnel_offset + sizeof(struct pfring_udphdr))
      return 1;

    udp = (struct pfring_udphdr *)(&data[tunnel_offset]);
    ...
    struct pfring_tcphdr *sctp;    /* We just want source and dest port here */

    if(data_len < tunnel_offset + 12)
      return 1;

    sctp = (struct pfring_tcphdr *)(&data[tunnel_offset]);
...
#define VLAN_VID_MASK 0x0fff
  ...
  if (hdr->extended_hdr.parsed_pkt.eth_type == 0x8100 /* 802.1q (VLAN) */) {
    ...
    if (hdr->extended_hdr.parsed_pkt.eth_type == ETH_P_8021Q /* 0x8100 802.1q (VLAN) */) { /* QinQ */
 ...
 L3:
  ...
  if (hdr->extended_hdr.parsed_pkt.eth_type == 0x0800 /* IPv4 */) {
    struct pfring_iphdr *ip;

    hdr->extended_hdr.parsed_pkt.ip_version = 4;

    if (data_len < hdr->extended_hdr.parsed_pkt.offset.l3_offset + sizeof(struct pfring_iphdr))
      goto TIMESTAMP;

    ip = (struct pfring_iphdr *)(&data[hdr->extended_hdr.parsed_pkt.offset.l3_offset]);
 ...
 L4:
  ...
  if(hdr->extended_hdr.parsed_pkt.l3_proto == IPPROTO_TCP) {
    ...
    if(data_len < hdr->extended_hdr.parsed_pkt.offset.l4_offset + sizeof(struct pfring_tcphdr))
      goto TIMESTAMP;

    tcp = (struct pfring_tcphdr *)(&data[hdr->extended_hdr.parsed_pkt.offset.l4_offset]);
    ...
    if(data_len < hdr->extended_hdr.parsed_pkt.offset.l4_offset + sizeof(struct pfring_udphdr))
      goto TIMESTAMP;

    udp = (struct pfring_udphdr *)(&data[hdr->extended_hdr.parsed_pkt.offset.l4_offset]);

    hdr->extended_hdr.parsed_pkt.l4_src_port = ntohs(udp->source), hdr->extended_hdr.parsed_pkt.l4_dst_port = ntohs(udp->dest);
    hdr->extended_hdr.parsed_pkt.offset.payload_offset = hdr->extended_hdr.parsed_pkt.offset.l4_offset + sizeof(struct pfring_udphdr);
	...
	if((hdr->extended_hdr.parsed_pkt.l4_src_port == GTP_U_DATA_PORT) ||
	  ...
	  if(gtp->flags & (GTP_FLAGS_EXTENSION | GTP_FLAGS_SEQ_NUM | GTP_FLAGS_NPDU_NUM)) {
	    ...
	    if((gtp->flags & GTP_FLAGS_EXTENSION) && gtpopt->next_ext_hdr) {
	      ...
	      do {
		if(data_len < (hdr->extended_hdr.parsed_pkt.offset.payload_offset+gtp_len +1/* 8bit len field */)) goto TIMESTAMP;
		...
		if(gtpext->len == 0 || data_len < (hdr->extended_hdr.parsed_pkt.offset.payload_offset+gtp_len)) goto TIMESTAMP;
		next_ext_hdr = (u_int8_t *) (&data[hdr->extended_hdr.parsed_pkt.offset.payload_offset + gtp_len - 1/* 8bit next_ext_hdr field*/]);
	      } while (*next_ext_hdr);
	    }
	  }

	  if(data_len < (hdr->extended_hdr.parsed_pkt.offset.payload_offset + gtp_len + sizeof(struct pfring_iphdr)))
	    goto TIMESTAMP;

	  tunneled_ip = (struct pfring_iphdr *) (&data[hdr->extended_hdr.parsed_pkt.offset.payload_offset + gtp_len]);
	...
	hdr->extended_hdr.parsed_pkt.tunnel.tunnel_id = ntohl(*tunnel_id);
      ...
      sctp = (struct pfring_tcphdr *)(&data[hdr->extended_hdr.parsed_pkt.offset.l4_offset]);
